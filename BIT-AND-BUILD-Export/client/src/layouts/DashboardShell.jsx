    ...
    <div className={`dashboard-shell dashboard-shell--${role}`}>
      ...
      <aside className="dashboard-shell__sidebar">
        <div className="dashboard-shell__sidebar-top">
          <Link to="/" className="dashboard-shell__wordmark">
            BIT <span>&amp;</span> BUILD
          </Link>
          <span className={`dashboard-shell__badge dashboard-shell__badge--${role}`}>
            {roleLabel}
          </span>
        </div>
        ...
        <nav className="dashboard-shell__nav">
          {navItems.map((item) => (
            <button
              key={item.id}
              className={`dashboard-shell__nav-item ${activeTab === item.id ? 'dashboard-shell__nav-item--active' : ''}`}
        ...
        <div className="dashboard-shell__sidebar-bottom">
          <div className="dashboard-shell__user-info">
            <div className="dashboard-shell__user-avatar">
          ...
          <button className="dashboard-shell__logout" onClick={handleLogout}>
            🚪 Logout
          </button>
        </div>
      </aside>
      ...
      <nav className="dashboard-shell__mobile-nav">
        {navItems.map((item) => (
          <button
            key={item.id}
            className={`dashboard-shell__mobile-tab ${activeTab === item.id ? 'dashboard-shell__mobile-tab--active' : ''}`}
            onClick={() => onTabChange(item.id)}
          >
            <span>{item.icon}</span>
