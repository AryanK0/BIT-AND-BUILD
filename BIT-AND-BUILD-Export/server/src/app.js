...
const presentationScore = z.object({ score: z.number().int().min(0).max(10), comments: z.string().trim().max(5000).optional().default('') }).strict();
...
export function createApp({ pool, config, logger = console } = {}) {
  ...
  // Presentations are kept in PostgreSQL rather than Render's ephemeral service
  // filesystem. Multer memory storage is bounded by the same 20 MB limit.
  const presentationUpload = multer({ storage: multer.memoryStorage(), limits: { fileSize: 20 * 1024 * 1024, files: 1 }, fileFilter: (_req, file, callback) => { if (isAllowedPresentationFile(file)) return callback(null, true); const error = new Error('Only PPT and PPTX files are allowed'); error.statusCode = 400; return callback(error); } });
  ...
  const presentationMetadata = `SELECT p.id,p.team_id,p.original_filename,p.stored_filename,p.mime_type,p.file_data,p.file_size,p.uploaded_at,p.updated_at,t.team_name FROM presentations p JOIN teams t ON t.id=p.team_id WHERE p.team_id=$1`;
  const sendPresentation = async (teamId, res, next, judgeDownload = false) => { try { const presentation = (await pool.query(presentationMetadata, [teamId])).rows[0]; if (!presentation) return res.status(404).json(fail(404)); const extension = path.extname(presentation.original_filename).toLowerCase(); const filename = judgeDownload ? `${safePresentationName(String(presentation.team_name || 'team'))}-presentation${extension}` : presentation.original_filename; res.type(presentation.mime_type); res.setHeader('Content-Disposition', `attachment; filename="${filename.replace(/["\\]/g, '')}"`); if (presentation.file_data) return res.send(presentation.file_data); // Legacy records are available only while their private local file remains present.
    const storedPath = path.resolve(config.uploadDir, path.basename(presentation.stored_filename)); if (!storedPath.startsWith(path.resolve(config.uploadDir) + path.sep) || !fs.existsSync(storedPath)) return res.status(404).json({ error: { code: 'PRESENTATION_FILE_UNAVAILABLE', message: 'This legacy presentation file is no longer available. Please ask the team to upload it again.' } }); return res.sendFile(storedPath); } catch (error) { return next(error); } };
  ...
  app.post('/api/presentations', ...role('participant'), presentationUpload.single('presentation'), async (req, res, next) => { let client; try { if (!req.file?.buffer) return res.status(400).json(fail(400)); requirePool(pool); client = await pool.connect(); await client.query('BEGIN'); const submissionRecord = (await client.query('SELECT status FROM submissions WHERE team_id=$1 FOR UPDATE', [req.session.teamId])).rows[0]; if (submissionRecord?.status === 'final') { await client.query('ROLLBACK'); return res.status(403).json(fail(403)); } const result = await client.query(`INSERT INTO presentations (team_id,original_filename,stored_filename,mime_type,file_data,file_size,file_checksum) VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (team_id) DO UPDATE SET original_filename=EXCLUDED.original_filename,stored_filename=EXCLUDED.stored_filename,mime_type=EXCLUDED.mime_type,file_data=EXCLUDED.file_data,file_size=EXCLUDED.file_size,file_checksum=EXCLUDED.file_checksum,uploaded_at=NOW(),updated_at=NOW() RETURNING original_filename,mime_type,uploaded_at`, [req.session.teamId, safePresentationName(req.file.originalname), `${crypto.randomUUID()}${path.extname(req.file.originalname).toLowerCase()}`, req.file.mimetype, req.file.buffer, req.file.size, crypto.createHash('sha256').update(req.file.buffer).digest('hex')]); await client.query('COMMIT'); res.status(201).json({ presentation: { originalFilename: result.rows[0].original_filename, mimeType: result.rows[0].mime_type, uploadedAt: result.rows[0].uploaded_at } }); } catch (error) { if (client) await client.query('ROLLBACK').catch(() => {}); next(error); } finally { client?.release(); } });
